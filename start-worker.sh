...
celery --app task_queue.app worker --queues Queue1 --hostname=worker1@%h --concurrency=2 --loglevel=INFO &
celery --app task_queue.app worker --queues Queue2 --hostname=worker2@%h --concurrency=1 --loglevel=INFO &
celery --app task_queue.app worker --queues Queue1,Queue2 --hostname=worker3@%h --concurrency=1 --loglevel=INFO &
